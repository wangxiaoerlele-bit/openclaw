...
describe("listFeishuAccountIds", () => {
  it("skips defaults-only accounts.default when named accounts are configured", () => {
    const cfg = {
      channels: {
        feishu: {
          defaultAccount: "wunian",
          accounts: {
            default: {
              dmPolicy: "allowlist",
              allowFrom: ["ou_owner"],
            ...
            wunian: { appId: "cli_wunian", appSecret: "secret_wunian" },
            xiaozhuli: { appId: "cli_xiaozhuli", appSecret: "secret_xiaozhuli" },
          },
        },
      },
    };

    expect(listFeishuAccountIds(cfg as never)).toEqual(["wunian", "xiaozhuli"]);
  });
});
...
describe("resolveFeishuAccount", () => {
  it("inherits shared defaults from defaults-only accounts.default", () => {
    const cfg = {
      channels: {
        feishu: {
          defaultAccount: "wunian",
    ...
    const account = resolveFeishuAccount({ cfg: cfg as never, accountId: "wunian" });

    expect(account.accountId).toBe("wunian");
    expect(account.appId).toBe("cli_wunian");
    expect(account.config.dmPolicy).toBe("allowlist");
    expect(account.config.allowFrom).toEqual(["ou_owner"]);
    expect(account.config.groupPolicy).toBe("allowlist");
    expect(account.config.groupAllowFrom).toEqual(["oc_group_1"]);
    expect(account.config.resolveSenderNames).toBe(false);
  });
