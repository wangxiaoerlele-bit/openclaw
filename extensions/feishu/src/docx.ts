...
const STANDARD_BASE64_RE = /^[A-Za-z0-9+/]+=*$/;
const RAW_JPEG_BASE64_PREFIX = "/9j/";

function isStandardBase64Payload(value: string): boolean {
  return value.length > 0 && STANDARD_BASE64_RE.test(value);
}

function looksLikePlainBase64Jpeg(value: string): boolean {
  const trimmed = value.trim();
  return (
    trimmed.length >= 32 &&
    trimmed.startsWith(RAW_JPEG_BASE64_PREFIX) &&
    isStandardBase64Payload(trimmed)
  );
}

function defaultBase64ImageFileName(trimmed: string): string {
  return trimmed.startsWith(RAW_JPEG_BASE64_PREFIX) ? "image.jpg" : "image.png";
}

function decodePlainBase64Image(
  imageInput: string,
  maxBytes: number,
  ...
  if (!isStandardBase64Payload(trimmed)) {
    throw new Error(
      `Invalid base64: image input contains characters outside the standard base64 alphabet. ` +
        `Use a data URI (data:image/png;base64,...) or file_path for local files instead.`,
    );
  }
  ...
  return {
    buffer,
    fileName: explicitFileName ?? defaultBase64ImageFileName(trimmed),
  };
}
...
async function resolveUploadInput(
  ...
  maxBytes: number,
  explicitFileName?: string,
  imageInput?: string, // data URI or plain base64; local files should use file_path
  ...
  if (imageInput?.startsWith("data:")) {
    ...
    if (!isStandardBase64Payload(trimmedData)) {
      throw new Error(
  ...
  // Legacy explicit local path support: ~, ./ and ../ are unambiguous.
  // Absolute paths (/...) are supported only when they exist on disk. Keep the
  // plain-base64 `image` contract authoritative, including raw JPEG payloads
  // that begin with "/9j/" and would otherwise resemble absolute paths.
  if (imageInput) {
    ...
    if (absolutePath && looksLikePlainBase64Jpeg(imageInput)) {
      return decodePlainBase64Image(imageInput, maxBytes, explicitFileName);
    }

    if (absolutePath && !existsSync(candidate)) {
      throw new Error(
        `File not found: "${candidate}". ` +
          `If you intended to pass image binary data, use the image field with a data URI or plain base64 string.`,
      );
    }
  }
