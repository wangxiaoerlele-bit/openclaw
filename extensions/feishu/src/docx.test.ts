...
describe("feishu_doc image fetch hardening", () => {
  ...
  it("uploads plain JPEG base64 via upload_image", async () => {
    const feishuDocTool = resolveFeishuDocTool();
    ...
    expect(result.details.file_name).toBe("image.jpg");
    expect(driveUploadAllMock).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          file_name: "image.jpg",
          parent_type: "docx_image",
          parent_node: "img_block_1",
        }),
      }),
    );
  });

  it("keeps non-existent absolute paths in image as file-path errors", async () => {
    const feishuDocTool = resolveFeishuDocTool();

    const result = await feishuDocTool.execute("tool-call", {
      action: "upload_image",
      doc_token: "doc_1",
      image: "/tmp/nonexistentimagepath",
    });

    expect(result.details.error).toContain('File not found: "/tmp/nonexistentimagepath"');
    expect(driveUploadAllMock).not.toHaveBeenCalled();
  });
