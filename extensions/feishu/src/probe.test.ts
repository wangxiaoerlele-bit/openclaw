...
function createAbortableRequestFn() {
  let requestSignal: AbortSignal | undefined;
  const requestFn = vi.fn().mockImplementation(({ signal }: { signal?: AbortSignal }) => {
...
describe("probeFeishu", () => {
  ...
  it("aborts the underlying request when probe times out", async () => {
    vi.useFakeTimers();
    try {
      const { requestFn, getRequestSignal } = createAbortableRequestFn();

      const probePromise = probeFeishu(
      ...
      expect(requestFn).toHaveBeenCalledTimes(1);
      expect(getRequestSignal()).toBeDefined();
  ...
  it("aborts the underlying request when the caller aborts mid-flight", async () => {
    const { requestFn, getRequestSignal } = createAbortableRequestFn();
    const abortController = new AbortController();

    const probePromise = probeFeishu(
      { appId: "cli_123", appSecret: "secret" },
      { abortSignal: abortController.signal, timeoutMs: 5_000 },
    );
    abortController.abort();

    await expect(probePromise).resolves.toMatchObject({ ok: false, error: "probe aborted" });
    expect(requestFn).toHaveBeenCalledTimes(1);
    expect(getRequestSignal()).toBeDefined();
    expect(getRequestSignal()?.aborted).toBe(true);
  });
