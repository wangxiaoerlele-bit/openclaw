...
const senderLookupPermissionBackoffUntil = new Map<string, number>();
...
function resolvePermissionCacheKey(account: ResolvedFeishuAccount): string {
  return account.appId ?? normalizeAccountId(account.accountId);
}
...
async function resolveFeishuSenderName(params: {
  ...
  const permissionCacheKey = resolvePermissionCacheKey(account);
  const backoffUntil = senderLookupPermissionBackoffUntil.get(permissionCacheKey) ?? 0;
  if (backoffUntil > now) {
    return {};
  }
  ...
  try {
    ...
    if (name && typeof name === "string") {
      ...
      senderLookupPermissionBackoffUntil.delete(permissionCacheKey);
      return { name };
    }
    ...
    if (permErr) {
      senderLookupPermissionBackoffUntil.set(
        permissionCacheKey,
        now + PERMISSION_ERROR_COOLDOWN_MS,
      );
...
function matchesFeishuBindingPeerKind(
  bindingKind: unknown,
  expectedKind: "group" | "direct",
): boolean {
  if (typeof bindingKind !== "string") {
    return false;
  }
  const normalized = bindingKind.trim().toLowerCase();
  if (expectedKind === "group") {
    return normalized === "group" || normalized === "channel";
  }
  return normalized === expectedKind;
}

// Prefer an explicit peer binding's account for replies so multi-account groups
// can route back through the intended bot instead of whichever account observed
// the inbound event first.
function resolvePreferredReplyAccountId(params: {
  cfg: ClawdbotConfig;
  agentId: string;
  inboundAccountId: string;
  peer: { kind: "group" | "direct"; id: string };
  parentPeer?: { kind: "group" | "direct"; id: string } | null;
}): string {
  const bindings = Array.isArray(params.cfg.bindings) ? params.cfg.bindings : [];
  const normalizedAgentId = normalizeAgentId(params.agentId);
  const peers = params.parentPeer ? [params.peer, params.parentPeer] : [params.peer];
  ...
  for (const candidatePeer of peers) {
    const matched = bindings.find((binding) => {
      ...
      if (peerId !== candidatePeer.id) {
        return false;
      }
      return matchesFeishuBindingPeerKind(peer.kind, candidatePeer.kind);
    });

    if (matched?.match?.accountId) {
      return normalizeAccountId(matched.match.accountId);
    }
  }

  return normalizeAccountId(params.inboundAccountId);
}
...
export async function handleFeishuMessage(params: {
  ...
  if (feishuCfg?.resolveSenderNames ?? true) {
    ...
    if (senderResult.permissionError) {
      const appKey = resolvePermissionCacheKey(account);
      const now = Date.now();
  ...
  try {
    ...
    // Direct chats must reply from the same bot account that received the DM.
    const preferredReplyAccountId = isGroup
      ? resolvePreferredReplyAccountId({
          cfg: effectiveCfg,
          agentId: route.agentId,
          inboundAccountId: account.accountId,
          peer: {
            kind: "group",
            id: peerId,
          },
          parentPeer:
            parentPeer?.id && parentPeer.kind === "group"
              ? { kind: "group", id: parentPeer.id }
              : undefined,
        })
      : normalizeAccountId(account.accountId);
    if (isGroup && preferredReplyAccountId !== normalizeAccountId(account.accountId)) {
      log(
        `feishu[${account.accountId}]: rerouting replies for ${route.sessionKey} via account ${preferredReplyAccountId}`,
      );
    }
    if (isGroup) {
      const replyAccountDedupKey = `reply:${preferredReplyAccountId}`;
      if (!(await tryRecordMessagePersistent(ctx.messageId, replyAccountDedupKey, log))) {
        log(
          `feishu[${account.accountId}]: reply account ${preferredReplyAccountId} already claimed message ${ctx.messageId}; skipping duplicate group dispatch`,
        );
        return;
      }
    }
    ...
    if (broadcastAgents) {
      ...
      const dispatchForAgent = async (agentId: string) => {
        ...
        const agentReplyAccountId = resolvePreferredReplyAccountId({
          cfg: effectiveCfg,
          agentId,
          inboundAccountId: account.accountId,
          peer: {
          ...
          parentPeer:
            isGroup && parentPeer?.id && parentPeer.kind === "group"
              ? { kind: "group", id: parentPeer.id }
              : undefined,
        });
      ...
      const { dispatcher, replyOptions, markDispatchIdle } = createFeishuReplyDispatcher({
        ...
        threadReply,
        mentionTargets: ctx.mentionTargets,
        accountId: preferredReplyAccountId,
        messageCreateTimeMs,
      });
