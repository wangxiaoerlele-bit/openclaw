...
describe("handleFeishuMessage command authorization", () => {
  ...
  it("prefers an explicit peer binding account for group replies", async () => {
    mockResolveAgentRoute.mockReturnValue({
      agentId: "main",
      channel: "feishu",
      accountId: "wunian",
      sessionKey: "agent:main:feishu:group:oc-group-bound-to-main",
      mainSessionKey: "agent:main:main",
      matchedBy: "default",
    });
    ...
    const cfg: ClawdbotConfig = {
      ...
      channels: {
        feishu: {
          ...
          groupAllowFrom: ["oc-group-bound-to-main"],
          groups: {
            "oc-group-bound-to-main": {
    ...
    const event: FeishuMessageEvent = {
      ...
      message: {
        message_id: "msg-group-preferred-account",
        chat_id: "oc-group-bound-to-main",
        chat_type: "group",
        message_type: "text",
  ...
  it("keeps direct-message replies on the inbound account even when another account is peer-bound", async () => {
    mockResolveAgentRoute.mockReturnValue({
      agentId: "main",
      channel: "feishu",
      accountId: "wunian",
      sessionKey: "agent:main:feishu:direct:ou-direct-bound-to-main",
      mainSessionKey: "agent:main:main",
      matchedBy: "default",
    });
    ...
    const cfg: ClawdbotConfig = {
      ...
      channels: {
        feishu: {
          dmPolicy: "open",
          allowFrom: ["ou-direct-bound-to-main"],
        },
      },
    } as ClawdbotConfig;
    ...
    const event: FeishuMessageEvent = {
      sender: {
        sender_id: {
          open_id: "ou-direct-bound-to-main",
        },
      },
      message: {
        message_id: "msg-direct-inbound-account",
        chat_id: "oc-direct-chat",
        chat_type: "p2p",
        message_type: "text",
  ...
  it("backs off sender-name lookups after permission errors", async () => {
    mockShouldComputeCommandAuthorized.mockReturnValue(false);
    const getUserMock = vi.fn().mockRejectedValue({
      response: {
        data: {
          code: 99991672,
          msg: "permission denied https://open.feishu.cn/app/cli_perm_backoff",
        },
      },
    });
    mockCreateFeishuClient.mockReturnValue({
    ...
    const cfg: ClawdbotConfig = {
      channels: {
        feishu: {
          appId: "cli_perm_backoff",
          appSecret: "sec_perm_backoff",
          groups: {
            "oc-group": {
              requireMention: false,
            },
    ...
    await dispatchMessage({
      ...
      event: {
        sender: {
          sender_id: {
            open_id: "ou-backoff-1",
          },
        },
        message: {
          message_id: "msg-perm-backoff-1",
          chat_id: "oc-group",
          chat_type: "group",
          message_type: "text",
          content: JSON.stringify({ text: "first group message" }),
        },
      },
    });
    await dispatchMessage({
      ...
      event: {
        sender: {
          sender_id: {
            open_id: "ou-backoff-2",
          },
        },
        message: {
          message_id: "msg-perm-backoff-2",
          chat_id: "oc-group",
          chat_type: "group",
          message_type: "text",
          content: JSON.stringify({ text: "second group message" }),
        },
      },
    });

    expect(getUserMock).toHaveBeenCalledTimes(1);
    expect(mockFinalizeInboundContext).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({
        BodyForAgent: expect.stringContaining(
          "Permission grant URL: https://open.feishu.cn/app/cli_perm_backoff",
        ),
      }),
    );
    expect(mockFinalizeInboundContext).toHaveBeenNthCalledWith(
