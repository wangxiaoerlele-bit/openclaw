...
function isSuiteEnabled(envVarName) {
  const raw = process.env[envVarName]?.trim().toLowerCase();
  return raw !== "0" && raw !== "false";
}

// Keep the default `pnpm test` path comprehensive. Callers can still disable
// suites explicitly (for example OPENCLAW_TEST_INCLUDE_GATEWAY=0) when needed.
const includeChannelsSuite = isSuiteEnabled("OPENCLAW_TEST_INCLUDE_CHANNELS");
const includeGatewaySuite = isSuiteEnabled("OPENCLAW_TEST_INCLUDE_GATEWAY");
const includeExtensionsSuite = isSuiteEnabled("OPENCLAW_TEST_INCLUDE_EXTENSIONS");
const runs = [
  ...(useVmForks
    ? [
        {
    ...
    : testProfile === "serial"
      ...
      : testProfile === "max"
        ? {
            ...
            channels: Math.max(1, Math.min(6, Math.floor(localWorkers / 2))),
        ...
        : highMemLocalHost
          ? {
              ...
              channels: Math.max(1, Math.min(4, Math.floor(localWorkers / 4))),
...
const maxWorkersForRun = (name) => {
  ...
  if (name === "channels") {
    return defaultWorkerBudget.channels;
  }
  if (name === "extensions") {
