...
describe("memory index", () => {
  ...
  it("does not reuse a status-only manager for normal search access", async () => {
    const cfg = createCfg({ storePath: indexStatusPath });
    ...
    const searchable = await getMemorySearchManager({ cfg, agentId: "main" });
    const searchManager = requireManager(searchable, "search manager missing");

    expect(searchManager).not.toBe(statusManager);
    expect(searchManager.status().dirty).toBe(true);
