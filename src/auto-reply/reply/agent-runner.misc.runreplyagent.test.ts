...
describe("runReplyAgent execution task guard", () => {
  it("creates a durable task for empty execution commitments", async () => {
    ...
    expect(result).toMatchObject({
      text: "已登记为持续任务并开始执行。后续只在有新事实、明确阻塞或完成时汇报。",
    });
  ...
  it("does not create a durable task after concrete mutating work already happened", async () => {
    ...
    const result = await createRunForGuard("main");

    expect(result).toMatchObject({
      text: "我会继续推进这条任务，刚刚已经把配置改掉了。",
    });
    expect(createTask).not.toHaveBeenCalled();
  });
});

function createRunForGuard(sessionKey: string) {
  const typing = createMockTypingController();
  const sessionCtx = {
    Provider: "telegram",
    OriginatingTo: "chat",
