...
describe("workspace path resolution", () => {
  ...
  it("prepends extra exec path entries for runtime-installed skill binaries", async () => {
    await withTempDir("openclaw-ws-", async (workspaceDir) => {
      const runtimeDir =
        process.platform === "win32" ? "C:\\skill-runtime\\bin" : "/skill-runtime/bin";
      const printPath = process.platform === "win32" ? "Write-Output $env:PATH" : "echo $PATH";
      const tools = createOpenClawCodingTools({
        workspaceDir,
        extraExecPathPrepend: [runtimeDir],
        exec: { host: "gateway", ask: "off", security: "full" },
      });
      const execTool = tools.find((tool) => tool.name === "exec");
      expect(execTool).toBeDefined();

      const result = await execTool?.execute("ws-exec-path", { command: printPath });
      const entries = getTextContent(result).trim().split(path.delimiter);

      expect(entries[0]).toBe(runtimeDir);
    });
  });
