...
Running `./gradlew` directly requires an Android SDK path to be configured first. Set
`ANDROID_SDK_ROOT` / `ANDROID_HOME` (macOS default is `~/Library/Android/sdk`) or create
`apps/android/local.properties` with `sdk.dir=/absolute/path/to/sdk`.
