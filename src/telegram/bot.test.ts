...
describe("createTelegramBot", () => {
  ...
  it("includes external reply image media in inbound context for text replies", async () => {
    onSpy.mockClear();
    replySpy.mockClear();
    getFileSpy.mockClear();
  ...
  it("routes direct-message reactions by sender id when chat id differs", async () => {
    onSpy.mockClear();
    enqueueSystemEventSpy.mockClear();
    ...
    expect(enqueueSystemEventSpy).toHaveBeenCalledWith(
      "Telegram reaction added: 👍 by Ada (@ada_bot) on msg 42",
      expect.objectContaining({
        sessionKey: expect.stringContaining("telegram:direct:9"),
        contextKey: expect.stringContaining("telegram:reaction:add:1234:42:9"),
      }),
    );
  });
