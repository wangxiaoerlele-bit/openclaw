...
describe("loadOpenClawPlugins", () => {
  ...
  it("builds scoped plugin-sdk aliases from available subpath files", () => {
    const root = makeTempDir();
    const compatSrcFile = path.join(root, "src", "plugin-sdk", "compat.ts");
    const coreSrcFile = path.join(root, "src", "plugin-sdk", "core.ts");
    const accountIdSrcFile = path.join(root, "src", "plugin-sdk", "account-id.ts");
    const telegramSrcFile = path.join(root, "src", "plugin-sdk", "telegram.ts");
    fs.mkdirSync(path.dirname(coreSrcFile), { recursive: true });
    fs.writeFileSync(compatSrcFile, "export {};\n", "utf-8");
    fs.writeFileSync(coreSrcFile, "export {};\n", "utf-8");
    fs.writeFileSync(accountIdSrcFile, "export {};\n", "utf-8");
    fs.writeFileSync(telegramSrcFile, "export {};\n", "utf-8");

    const aliasMap = withEnv({ NODE_ENV: undefined }, () =>
      __testing.resolvePluginSdkScopedAliasMap(path.join(root, "src", "plugins", "loader.ts")),
    );

    expect(aliasMap).toEqual({
      "openclaw/plugin-sdk/account-id": accountIdSrcFile,
      "openclaw/plugin-sdk/compat": compatSrcFile,
      "openclaw/plugin-sdk/core": coreSrcFile,
      "openclaw/plugin-sdk/telegram": telegramSrcFile,
    });
  });
});
